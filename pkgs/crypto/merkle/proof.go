...
func (poz ProofOperators) Verify(root []byte, keypath string, args [][]byte) (err error) {
	...
	for i, op := range poz {
		...
		if len(key) != 0 {
			if len(keys) == 0 {
				return errors.New("Key path has insufficient # of parts: expected no more keys but got %+v", string(key))
			}
			lastKey := keys[len(keys)-1]
			if !bytes.Equal(lastKey, key) {
				return errors.New("Key mismatch on operation #%d: expected %+v but got %+v", i, string(lastKey), string(key))
			}
	...
	if !bytes.Equal(root, args[0]) {
		return errors.New("Calculated root hash is invalid: expected %+v but got %+v", root, args[0])
	}
	if len(keys) != 0 {
